...
export default function EditOrderPage() {
  ...
  const loadOrder = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // 明細を設定
        const newDetails = data.details.map((detail: OrderDetailType) => ({
  ...
  const onSubmit = async (data: OrderFormData) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(
          error.error ||
            "他のユーザーによって更新されています。最新のデータを取得してください。"
        );
        loadOrder();
      } else {
        alert("更新に失敗しました");
  ...
  return (
    <div className="px-4 py-6">
      <h3 className="text-2xl font-bold mb-4 text-gray-900">受注修正・確認</h3>
          ...
          <table className="table table-bordered table-sm">
            <thead className="bg-gray-100">
              <tr>
                ...
                <th className="text-center p-2" style={{ width: "12%" }}>
                  商品コード
                </th>
                <th className="text-center p-2" style={{ width: "28%" }}>
                  商品名
                </th>
                <th className="text-center p-2" style={{ width: "13%" }}>
                  単価
                </th>
                <th className="text-center p-2" style={{ width: "13%" }}>
                  数量
                </th>
                <th className="text-center p-2" style={{ width: "18%" }}>
                  金額
                </th>
              </tr>
            </thead>
